...
pub async fn run(gateway: &str, client: &reqwest::Client, args: PolicyArgs) -> anyhow::Result<()> {
        ...
        || args.file.extension().and_then(|s| s.to_str()) == Some("yml")
    {
        ...
        serde_json::from_str(&content).or_else(|_| -> anyhow::Result<serde_json::Value> {
            ...
            serde_json::to_value(yaml_fallback)
                .map_err(|e| anyhow::anyhow!("YAML fallback conversion failed: {}", e))
        })?
    };
