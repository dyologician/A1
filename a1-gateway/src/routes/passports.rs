...
fn ttl_str_to_secs(s: &str) -> Option<u64> {
    ...
    if let Some(y) = s.strip_suffix('y') {
        return y.parse::<u64>().ok().map(|n| n * 31536000);
    }
    s.parse::<u64>().ok()
}
    ...
    let ttl_secs = match ttl_str_to_secs(&req.ttl) {
        ...
        _ => {
            return Json(IssuePassportResponse {
                ...
                expires_at: String::new(),
                error: Some(format!(
                    "Invalid TTL '{}'. Use '30d', '7d', '1y', '3600' etc.",
                    req.ttl
                )),
            })
        }
    ...
    match DyoloPassport::issue(&ns, &caps_refs, ttl_secs, &identity, &SystemClock) {
        ...
        Ok(passport) => {
            ...
            let expires_at = {
                ...
                format!(
                    "Unix timestamp: {unix}  (~{} days from now)",
                    ttl_secs / 86400
                )
            };
...
pub async fn renew_passport_handler(
    ...
    issue_passport_handler(state, Json(issue_req))
        .await
        .into_response()
}
        ...
        match std::fs::write(&plist_path, &plist) {
            Ok(()) => {
                ...
                return Json(AutostartResponse {
                    ...
                    message: "A1 will now start automatically every time you log in to your Mac."
                        .into(),
                    error: None,
                })
                .into_response();
...
pub async fn explain_error_handler(
    ...
    let e: Entry = if err.contains("capability not granted")
        || err.contains("capabilitynotgranted")
        || code == "E4003"
    {
        Entry {
        ...
        Entry {
            ...
            cause:
                "The certificate was explicitly revoked, or the passport file was manually removed.",
    ...
    } else if err.contains("missing chain")
        || err.contains("missing signed chain")
        || code == "E4002"
    {
        Entry {
...
pub async fn read_passport_handler(
    ...
    let requested = match params.get("path") {
        ...
        _ => {
            return (
                StatusCode::BAD_REQUEST,
                axum::Json(serde_json::json!({"error":"path required"})),
            )
                .into_response()
        }
    };
        ...
        .and_then(|s| serde_json::from_str::<serde_json::Value>(&s).ok())
    {
        ...
        None => (
            ...
            axum::Json(serde_json::json!({"error":"invalid passport file"})),
        )
            .into_response(),
    }
}
...
pub async fn restore_passports_handler(
    ...
    if let Err(e) = std::fs::create_dir_all(&dir) {
        return Json(RestoreResponse {
            ...
            restored: 0,
            skipped: 0,
            error: Some(format!("Cannot create passports dir: {e}")),
        });
    }
    ...
    for pp in req.passports {
        ...
        if std::fs::write(&dest, json).is_ok() {
            restored += 1;
        } else {
            skipped += 1;
        }
