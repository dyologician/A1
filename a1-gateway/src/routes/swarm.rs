...
fn add_member_inner(req: AddMemberRequest) -> anyhow::Result<serde_json::Value> {
    ...
    let pk_arr: [u8; 32] = pk_bytes
        .try_into()
        .map_err(|_| anyhow::anyhow!("agent_pk_hex must be 32 bytes"))?;
    ...
    let role = match req.role.as_str() {
        ...
        "supervisor" => SwarmRole::Supervisor {
            capabilities: req.capabilities.clone(),
            max_worker_ttl_secs: req.ttl_seconds,
        },
        _ => SwarmRole::Worker {
            capabilities: req.capabilities.clone(),
        },
    ...
    let swarm = store_lock
        .get_mut(&req.swarm_id)
...
pub async fn remove_member_handler(Json(req): Json<RemoveMemberRequest>) -> impl IntoResponse {
    ...
    match store_lock.get_mut(&req.swarm_id) {
        None => (
            ...
            Json(serde_json::json!({ "error": "swarm not found" })),
        )
            .into_response(),
        Some(swarm) => match swarm.remove_member(&req.agent_did) {
            ...
            Some(_) => (
                StatusCode::OK,
                Json(serde_json::json!({ "removed": req.agent_did })),
            )
                .into_response(),
        },
    }
...
pub async fn list_members_handler(Path(swarm_id): Path<String>) -> impl IntoResponse {
    ...
    match store_lock.get(&swarm_id) {
        ...
        Some(swarm) => {
            ...
            (
                StatusCode::OK,
                Json(serde_json::json!({ "members": members, "count": members.len() })),
            )
                .into_response()
        }
    }
