...
pub async fn enable_autostart(State(_s): State<Arc<AppState>>) -> Json<SimpleResult> {
    match try_enable_autostart() {
        Ok(msg) => Json(SimpleResult {
            success: true,
            message: msg,
        }),
        Err(msg) => Json(SimpleResult {
            success: false,
            message: msg,
        }),
    }
...
<dict>
  ...
  <key>StandardErrorPath</key> <string>{home}/.a1/gateway.log</string>
        ...
        std::fs::write(&plist_path, plist).map_err(|e| format!("Could not write plist: {e}"))?;
...
async fn run_docker_install() {
    #[cfg(target_os = "macos")]
    {
        ...
        let _ = Command::new("curl")
            .args(["-fL", "--progress-bar", &url, "-o", tmp])
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .status();
        let _ = Command::new("hdiutil")
            .args(["attach", tmp, "-quiet", "-nobrowse"])
            .status();
